...
class AuthService:
    ...
    def authenticate(db: Session, *, email: str, password: str) -> str:
        ...
        token = create_access_token(sub=str(user.id), org_id=user.org_id, role=user.role.value)
        return token

    @staticmethod
    def register_and_create_org(db: Session, *, org_name: str, email: str, password: str, full_name: str | None):
