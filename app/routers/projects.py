...
from app.schemas.project import ProjectCreate, ProjectOut, ProjectAddMembersRequest
...
def create_project(payload: ProjectCreate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    ProjectMemberRepository.add_member(db, project_id=project.id, user_id=current_user.id)
    return project
...
def add_project_members(project_id: int, payload: ProjectAddMembersRequest, db: Session = Depends(get_db), current_user = Depends(require_roles("admin", "manager"))):
    project = ProjectRepository.get_by_id(db, project_id)
    if not project or project.org_id != current_user.org_id:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    for user_id in payload.user_ids:
        ...
        if not ProjectMemberRepository.is_member(db, project_id, user_id):
            ProjectMemberRepository.add_member(db, project_id=project_id, user_id=user_id)
...
def remove_project_member(project_id: int, user_id: int, db: Session = Depends(get_db), current_user = Depends(require_roles("admin", "manager"))):
...
def list_project_members(project_id: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    project = ProjectRepository.get_by_id(db, project_id)
    if not project or project.org_id != current_user.org_id:
        raise HTTPException(status_code=404, detail="Project not found")
